...
let imagesLoaded = 0;
const imagesToLoad = [
    yardImg, doghouseImg, treeImg, keritesImg, postmanImg,
...
function gameLoop() {
    ...
    if (yardImg.complete && yardImg.naturalWidth > 0) {
    ...
    if (dog.isDead) {
        let deadImg = dogImages.dead.complete && dogImages.dead.naturalWidth > 0 ? dogImages.dead : dogImages.angry;
        ctx.drawImage(deadImg, dog.x, dog.y, dog.width, dog.height);
    } else {
        if (dog.currentImage && dog.currentImage.complete && dog.currentImage.naturalWidth > 0) {
...
function drawHUD() {
    if (dog.isDead) {
        ...
        let deadImg = dogImages.dead.complete && dogImages.dead.naturalWidth > 0 ? dogImages.dead : dogImages.angry;
        ctx.drawImage(deadImg, canvas.width / 2 - 125, canvas.height / 2 - 320, 250, 250);
